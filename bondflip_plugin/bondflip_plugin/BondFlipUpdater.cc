...
/*! \file BondFlipUpdater.cc
    \brief Definition of BondFlipUpdater
*/

// ********************************
// here follows the code for BondFlipUpdater on the CPU
...
{    
    ...
    if (eva_old && eva_new)
    {
        ...
        if (m_vertex_bond_num[a] > m_Na && m_vertex_bond_num[b] > m_Na && m_vertex_bond_num[c] < m_Nb && m_vertex_bond_num[d] < m_Nb)
            if (mb_stats > temp_rand)
            {
                ...
                m_accepted++;
                m_accepted_bonds.push_back(curr_b);
                return;
...
*/
void BondFlipUpdater::update(uint64_t timestep)
    {
        
        Updater::update(timestep);

        // m_accepted = 0;
        // m_accepted_bonds.clear();
...
std::string BondFlipUpdater::get_accepted_bonds()
    {
        std::ostringstream oss;
        for (size_t i = 0; i < m_accepted_bonds.size(); i++)
        {
            if (i > 0) oss << ",";  // delimiter
            oss << m_accepted_bonds[i];
        }
        return oss.str();
    }

Scalar BondFlipUpdater::get_num_accepted_bonds()
    {
        return m_accepted;
    }
...
void export_BondFlipUpdater(pybind11::module& m)
    {
    pybind11::class_<BondFlipUpdater, Updater, std::shared_ptr<BondFlipUpdater> >(m, "BondFlipUpdater")
        ...
        .def("get_accepted_bonds", &BondFlipUpdater::get_accepted_bonds)
        .def("get_num_accepted_bonds", &BondFlipUpdater::get_num_accepted_bonds)
...
BondFlipUpdaterGPU::BondFlipUpdaterGPU(std::shared_ptr<SystemDefinition> sysdef,
                                        ...
                                        std::map<unsigned int, unsigned int> bond_dihedral,
                                        std::map<unsigned int, std::vector<unsigned int>> bond_faces)
    : BondFlipUpdater(sysdef, trigger, bond_neigh_bonds, bond_dihedral, bond_faces)
    {
...
/*! Perform the needed calculations to zero the system's velocity
    \param timestep Current time step of the simulation
*/
void BondFlipUpdaterGPU::update(uint64_t timestep)
    {
    Updater::update(timestep);
...
void export_BondFlipUpdaterGPU(pybind11::module& m)
    {
    pybind11::class_<BondFlipUpdaterGPU, BondFlipUpdater, std::shared_ptr<BondFlipUpdaterGPU>>(
        m, "BondFlipUpdaterGPU")
        .def(pybind11::init<std::shared_ptr<SystemDefinition>,
        std::shared_ptr<Trigger>,
        // std::shared_ptr<ComputeThermo>,
        // std::shared_ptr<Logger>,
