...
class mem():
    ...
    def update_bond_from_file(self):
        with open('bond_neigh_bonds.json') as f:
            data = json.load(f)
            self.bond_neigh_bonds = {int(k): [int(i) for i in v] for k, v in data.items()}
        with open('bond_dihedrals.json') as f:
            data = json.load(f)
            self.bond_dihedrals = {int(k): int(v) for k, v in data.items()}
        with open('bond_faces.json') as f:
            data = json.load(f)
            self.bond_faces = {int(k): [int(i) for i in v] for k, v in data.items()}
    ...
    def frame_in(self):
        with open('../{}.csv'.format(self.param['memfile']),'r') as f:
            frame_row = 0
            for row_num, line in enumerate(f, 1):
                if line[0] == '#':
                    ...
                    df = pd.read_csv('../{}.csv'.format(self.param['memfile']), skiprows = data_row, index_col = 0,header = 0, nrows = int(data_nrow),on_bad_lines = 'skip')
                    print(len(df))
                    print(df)
